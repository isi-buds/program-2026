...
##
path <- "/Users/dgillen/SynologyDrive/Teaching/ISI_BUDS/2024/Lecture2_DAGS_Confounding/"
fev <- read.table( paste(path,"fev.raw",sep=""), header=TRUE )
summary( fev )
fev$male <- as.numeric(as.factor(fev$sex)) - 1
table( fev$sex, fev$male )
fev$smoke <- as.numeric(as.factor(fev$smoke)) - 1
...
##
#####	Additional adjustment for sex as a (potential?) confounding variable
##
fit.sex <- lm( logfev ~ smoke + age + loght + male, subset=age>=9, data=fev )
summary( fit.sex )
lmCI( fit.sex, expcoef=TRUE )
cbind( summary(fit.sex)$sigma, summary(fit.adj)$sigma )
