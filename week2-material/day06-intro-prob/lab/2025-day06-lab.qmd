...
(NOTE): the function we implemented takes `variance` as an argument to parametrize the Normal distribution, while `rnorm` takes `sd`.
Indeed:
...
Compare the mean number of successes, first from 10 realizations with probability of successs p = 0.2 and 10 realizations with probability of success p = 0.6 ; and then 1000 realizations with each of p=0.2 and p = 0.6.
What do you observe?
...
**Note**: you could do it with a `sample()` function, but we don't want you to use it for this exercise.
...
For many common distributions, you can use `base R` or common packages to simulate random samples.
Generate 1000 samples and show the results using an appropriate plot from :
