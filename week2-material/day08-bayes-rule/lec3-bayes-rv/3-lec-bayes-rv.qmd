...
- We denote an observed _outcome_ of $Y$ using lower case $y$.  
...
Let Y represent a random variable that represents the number of applicants admitted to a PhD program which has received applications from 5 prospective students. That is $\Omega_Y = \{0, 1, 2, 3, 4, 5\}$. 

. . . 

We are interested in the parameter $\pi$ which represents the probability of acceptance to this program. 

. . . 

For demonstrative purposes, we will only consider three possible values of $\pi$ as 0.2, 0.4, and 0.8. 

## Prior model for $\pi$

We consult with an expert who knows the specific PhD program well. The following is the prior distribution the expert suggests we use in your analysis. 
...
Bayesian statistics is a balancing act and we will take both the prior and the data to get to the posterior. 
...
For the two scenarios, fill out the table:
...
Let's consider $\pi = 0.2$. 

. . .

If $\pi$ were 0.2 what is the probability that we would observe 4 of the 5 applicants get admitted to the program? 

. . .

Would you expect this probability to be high or low?
...
Let random variable $Y$ be the _number of successes_ (eg: number of accepted applicants) in $n$ _trials_ (eg: applications).  

. . .

Assume that the number of trials is _fixed_, the trials are _independent_, and the _probability of success_ (eg: probability of acceptance) in each trial is $\pi$.  

. . .

Then the _dependence_ of $Y$ on $\pi$ can be modeled by the Binomial model with __parameters__ $n$ and $\pi$.  In mathematical notation:
 
$$Y | \pi \sim \text{Bin}(n,\pi) $$

## The Binomial Model

The Binomial model is specified by a conditional pmf:    
...
If $\pi$ were 0.2, what is the probability that we would observe 3 of the 5 applicants get admitted to the program? 
