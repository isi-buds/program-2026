...
Find the 4th moment of a beta distribution with parameters alpha = 2, beta = 5.
Use:
...
b.  Monte Carlo integration to approximate it Hint: Uniform sampling won't work, because uniform distribution is not defined on $[0, \infty]$.
    For importance sampling to work, the support must be the same.
    Use exponential distribution samples instead -- think about how to write the desired integral in terms of expectation
...
Let Z have a standard normal distribution, $Z \sim N(0,1)$.
We want to compute $P(X \geq 4.5)$.
We could do this via a Naive Monte Carlo simulation.
(How?)
...
We generate a bunch of samples of Z and count how many satisfy $Z \geq 4$.
The problem is that there won’t be very many (perhaps none), unless my number of samples is very large.
...
We need to only consider the values where $X>8$ for the I.S.
sample (assing 0 to the others).
